class AgentController {
   ...
   * Setup is a function that define variables problem and wordl0 as parameter.problem and parameter.world
   * respectively. Also, it saves parameter.world into this.data.world.
   * JSON.stringify converts an object or value of Javascript into a string JSON
   * JSON.parse converts text into a JS object
  ...
  next() {
    if (!this.problem.goalTest(this.data)) {
      ...
      agent.receive(
        this.problem.perceptionForAgent(this.getData(), agent.getID())
      );
      // Espera
  ...
  /**
   * Return a copy of the agent controller data. The returned object contains the data of the problem (world) and the
